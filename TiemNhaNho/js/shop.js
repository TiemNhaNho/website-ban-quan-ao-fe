...
function applyFiltersAndSort() {
  ...
  if (currentFilters.categoryId) {
    ...
    console.log(
      "Matched products:",
      filteredProducts.map((p) => p.product_name)
    );
  }
  ...
  if (currentFilters.minPrice !== null || currentFilters.maxPrice !== null) {
    ...
    filteredProducts = filteredProducts.filter((product) => {
      const variant = allProductsData.variants.find(
  ...
  if (currentFilters.sortBy) {
    ...
    filteredProducts.sort((a, b) => {
      switch (currentFilters.sortBy) {
        ...
        case "price-asc":
        case "price-desc": {
          const variantA = allProductsData.variants.find(
            (v) => v.product_id === a.product_id
          );
          const variantB = allProductsData.variants.find(
            (v) => v.product_id === b.product_id
          );
          const priceA = variantA?.price_out ?? 0;
          const priceB = variantB?.price_out ?? 0;
          return currentFilters.sortBy === "price-asc"
            ? priceA - priceB
            : priceB - priceA;
        }
        default:
  ...
  if (filteredProducts.length === 0) {
    container.innerHTML =
      '<div class="col-12 text-center"><p>No products found matching your filters.</p></div>';
  ...
  container.innerHTML = filteredProducts
    .map((product) => {
      ...
      const image = allProductsData.images.find(
  ...
  console.log(
    `✓ Showing ${filteredProducts.length} of ${allProductsData.products.length} products`
  );
}
...
async function loadProducts(categoryId = null) {
  ...
  // Show loading state
  container.innerHTML =
    '<div class="col-12 text-center"><p>Loading products...</p></div>';
  ...
  try {
    ...
    if (!products || products.length === 0) {
      container.innerHTML =
        '<div class="col-12 text-center"><p>No products found in this category.</p></div>';
      return;
    }
    ...
    container.innerHTML =
      '<div class="col-12 text-center"><p class="text-danger">Failed to load products. Please try again later.</p></div>';
  }
}
...
async function loadCategoriesAndSetupTabs() {
  try {
    ...
    allTab.classList.add("tab"); // class currently used
    allTab.dataset.tabTarget = "#all"; // keep compatibility if needed by other scripts, though we override click
    ...
    categories.forEach((category) => {
      ...
      // If current filter matches this category, mark it as active
    ...
    console.log("=== Dynamic tabs setup complete ===");
  } catch (err) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const priceRangeRadios = document.querySelectorAll(
